AVPlayerViewController的使用方法

（1）使用AVPlayerViewController需要导入#import <AVKit/AVKit.h>头文件
...
（7）AVPlayerViewController中还有以下属性提供给开发者进行视频播放器属性的自定义：
...
（8）对于画中画的相关操作，开发者可以在程序中使用代理方法进行监听。AVPlayerViewController对象中有delegate这样一个属性，在ViewController.m中遵守如下协议：
	@interface ViewController() <AVPlayerViewControllerDelegate>
	@end

（9）开发者通过AVPlayerViewControllerDelegate协议中约定的方法来进行用户画中画操作的监听：
	// 将要进入画中画模式时系统调用的方法
...
（10）注意：AVPlayerViewControllerDelegate协议中约定的方法只能在iOS 9.0系统之后可用。